...
async function getprog(student_id, advisor_url_id){
    ...
    try{
        ...
        iddata = await connection.query("SELECT DISTINCT * FROM etm.student s, etm.advises a, etm.advisor ad WHERE ad.ID = a.advisorid and s.PID = a.pid and s.PID = ?;", student_id, (err, studentdata) => {
            if(err){
                console.log(err);
                return res.redirect('/errorpage_prog_study');
            }
        ...
        placeholderdata = await connection.query("SELECT * FROM `etm`.`placeholder` WHERE catalog = ?; SELECT * FROM `etm`.`catalog`; SELECT * FROM etm.course; SELECT * FROM etm.advisor; SELECT * FROM etm.heading where catalogYear = ? ORDER BY position; SELECT distinct semester FROM etm.enrolled_in WHERE TRIM(semester) <> '' ORDER BY substring(semester, 3, 4) DESC, substring(semester, 1, 2) ASC;", [iddata[0][0].current_catalog, iddata[0][0].current_catalog], (err, placing) => {
            if(err){    
                console.log(err);
                return res.redirect('/errorpage_prog_study');
            }
